...
import {
  FaEdit, FaTrash, FaPlus, FaSearch, FaCheck,
  FaExclamationCircle, FaArrowLeft, FaTag, FaList,
  FaFolder, FaFolderOpen, FaPlusCircle, FaMinusCircle
} from "react-icons/fa";
import { toast } from "react-toastify";
...
export default function ItemGroupPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = { 
        ...
        code, 
        category, 
        subgroups: subgroups.filter(s => s.trim()) 
      };
      
      if (editId) {
        const res = await axios.put(`/api/itemGroups/${editId}`, payload, { 
          headers: { Authorization: `Bearer ${token}` } 
        });
        if (res.data.success) { 
          setItemGroups(p => p.map(g => g._id === editId ? res.data.data : g)); 
          toast.success("Item Group updated!"); 
        }
      } else {
        const res = await axios.post("/api/itemGroups", payload, { 
          headers: { Authorization: `Bearer ${token}` } 
        });
        if (res.data.success) { 
          setItemGroups(p => [...p, res.data.data]); 
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (res.data.success) { 
        setItemGroups(p => p.filter(g => g._id !== id)); 
  ...
  const filtered = itemGroups.filter(g =>
    ...
    g.subgroups?.some(s => s.toLowerCase().includes(searchTerm.toLowerCase()))
  );
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 mb-5">
          {[
            { label: "Total Groups", value: itemGroups.length, emoji: "🏷️" },
            { label: "With Code", value: itemGroups.filter(g => g.code?.trim()).length, emoji: "🔖" },
            { label: "With Category", value: itemGroups.filter(g => g.category?.trim()).length, emoji: "📁" },
            { label: "With Subgroups", value: itemGroups.filter(g => g.subgroups?.length > 0).length, emoji: "📂" },
          ].map(s => (
            <div key={s.label} className="bg-white rounded-2xl p-4 flex items-center gap-3 border-2 border-transparent shadow-sm hover:-translate-y-0.5 hover:border-indigo-100 transition-all cursor-default">
              <span className="text-2xl">{s.emoji}</span>
              <div>
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100">
            <div className="relative flex-1 min-w-[180px] max-w-xs">
              ...
              <input
                ...
                value={searchTerm}
                onChange={e => setSearchTerm(e.target.value)}
                placeholder="Search by name, code, category or subgroup…"
              />
            </div>
            <p className="ml-auto text-xs text-gray-400 font-semibold whitespace-nowrap">{filtered.length} result{filtered.length !== 1 ? "s" : ""}</p>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  {["", "Group Name", "Code", "Category", "Subgroups", "Created", "Actions"].map(h => (
                    <th key={h} className="px-4 py-3 text-left text-[10.5px] font-bold uppercase tracking-wider text-gray-400 whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {loading ? (
                  ...
                  const isExpanded = expandedGroups[g._id] || false;
                  ...
                  return (
                    <React.Fragment key={g._id}>
                      <tr className="border-b border-gray-50 hover:bg-indigo-50/30 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            {hasSubgroups && (
                              ...
                              >
                                {isExpanded ? <FaFolderOpen className="text-xs" /> : <FaFolder className="text-xs" />}
                              </button>
                            )}
                            <p className="font-bold text-gray-900 text-sm">{g.name}</p>
                          </div>
                        ...
                        <td className="px-4 py-3">
                          {hasSubgroups ? (
                            <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-purple-600 bg-purple-50 px-2 py-0.5 rounded-full">
                              <FaFolder className="text-[9px]" />
                              {g.subgroups.length} subgroup{g.subgroups.length > 1 ? 's' : ''}
                            </span>
                          ) : (
                            <span className="text-gray-300 text-xs font-medium">—</span>
                          )}
                      ...
                      {/* Subgroups expanded view */}
                      {hasSubgroups && isExpanded && (
                        <tr>
                          <td colSpan={7} className="px-4 py-2 bg-purple-50/30">
                            <div className="ml-12">
                              <p className="text-[10px] font-bold uppercase tracking-wider text-purple-600 mb-2">Subgroups:</p>
                              <div className="flex flex-wrap gap-2">
                                {g.subgroups.map((sg, idx) => (
                                  <span key={idx} 
                                    className="inline-flex items-center gap-1.5 text-xs font-medium text-purple-700 bg-purple-100 px-2.5 py-1 rounded-full"
                                  >
                                    <FaFolder className="text-[8px]" />
                                    {sg}
                                  </span>
                                ))}
                              </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onKeyPress={e => { if (e.key === 'Enter') { e.preventDefault(); addSubgroup(); } }}
                  placeholder="Add subgroup name..."
                  className="flex-1 px-3 py-2.5 rounded-lg border border-gray-200 bg-white text-sm font-medium focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 outline-none transition-all placeholder:text-gray-300"
                />
                <button
                  onClick={addSubgroup}
                  className="px-4 py-2.5 rounded-lg bg-indigo-600 text-white text-sm font-semibold hover:bg-indigo-700 transition-all flex items-center gap-1.5 whitespace-nowrap"
                >
                  <FaPlusCircle className="text-xs" /> Add
                </button>
              </div>
              <p className="text-[11px] text-gray-400 mt-1">Press Enter or click Add to add a subgroup</p>
              ...
              {subgroups.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-2">
                  {subgroups.map((sg, index) => (
                    <span key={index} 
                      className="inline-flex items-center gap-1.5 text-xs font-medium text-purple-700 bg-purple-50 px-2.5 py-1 rounded-full border border-purple-200"
                    >
                      <FaFolder className="text-[8px]" />
                      {sg}
                      <button
