...
export default function RateTargetDetailPage() {
  ...
  const { canEdit, canView, loading: permissionLoading } = usePermission();
  ...
  useEffect(() => {
    if (!permissionLoading && (canView(MODULE) || canEdit(MODULE))) load();
  }, [id, permissionLoading, canView, canEdit]);

  const save = async () => {
    if (!canEdit(MODULE)) {
      setError('You do not have permission to edit Rate Target.');
      return;
    }
    if (!(Number(form.maxRate) > 0) || !(Number(form.targetRate) > 0)) {
      setError('Max Rate and Target Rate are required and must be greater than zero.');
      return;
    }
  ...
  const canEditRateTarget = canEdit(MODULE);
  const field = (key, label, type = 'text', required = false) => <label className="block"><span className="text-xs font-bold text-slate-600">{label}{required ? ' *' : ''}</span><input type={type} min={type === 'number' ? '0.01' : undefined} step={type === 'number' ? '0.01' : undefined} value={form[key]} disabled={!canEditRateTarget || saving} required={required} onChange={(e) => setForm((old) => ({ ...old, [key]: e.target.value }))} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm disabled:cursor-not-allowed disabled:bg-slate-100" /></label>;

  if (!permissionLoading && !canView(MODULE) && !canEditRateTarget) return <div className="p-8 text-sm text-red-600">You do not have permission to view Rate Target.</div>;
  ...
  return <div className="min-h-screen bg-slate-50 p-6"><div className="mx-auto max-w-4xl">
    ...
    {record && <><div className="mb-5 rounded-xl border border-slate-200 bg-white p-5"><h1 className="text-xl font-extrabold text-slate-900">Rate Target: {record.vnnNo}</h1><p className="mt-1 text-sm text-slate-500">{record.partyName || 'No party'} · {record.branchName || 'No branch'} · Part 1 locked</p></div>
      <section className="rounded-xl border border-slate-200 bg-white p-5"><div className="mb-4"><h2 className="font-bold text-slate-900">Rate Target Details</h2><p className="mt-1 text-xs text-slate-500">Max Rate and Target Rate are required. A successful save completes Part 2 and unlocks Vehicle Negotiation Part 3.</p></div>
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">{field('maxRate', 'Max Rate', 'number', true)}{field('targetRate', 'Target Rate', 'number', true)}{field('oldRatePercent', 'Old Rate %', 'number')}</div>
        <label className="mt-4 block"><span className="text-xs font-bold text-slate-600">Remarks</span><textarea value={form.remarks1} disabled={!canEditRateTarget || saving} onChange={(e) => setForm((old) => ({ ...old, remarks1: e.target.value }))} rows="3" className="mt-1 w-full rounded-lg border border-slate-300 p-3 text-sm disabled:bg-slate-100" /></label>
        <label className="mt-4 block"><span className="text-xs font-bold text-slate-600">Voice Note</span><input type="file" accept="audio/*" disabled={!canEditRateTarget || saving} onChange={uploadVoice} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm disabled:bg-slate-100" />{form.voiceNoteFile?.originalName && <span className="mt-1 block text-xs text-slate-500">Uploaded: {form.voiceNoteFile.originalName}</span>}</label>
        {canEditRateTarget && <button disabled={saving} onClick={save} className="mt-5 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-bold text-white disabled:bg-slate-400">Save Rate Target</button>}
      </section>
    </>}
  </div></div>;
}
