  ...
  if (pincode && pincode.length === 6) {
    ...
    if (result) {
      if (result.hasMultiple && result.allLocations && result.allLocations.length > 0) {
        ...
        // NEW: Update local status after state changes
...
const recalculatePalletizationWeights = (row) => {
 ...
 const addRow = () => {
  // Check if there's only one row and it's the default Palletization row
  if (packRows.length === 1 && packRows[0].packType === "PALLETIZATION") {
    // Replace the single row with the new pack type
    setPackRows([
      { ...defaultRow(activePack), packType: activePack }
    ]);
  } else {
    // Add new row alongside existing ones
    setPackRows((prev) => [
      ...prev,
      { ...defaultRow(activePack), packType: activePack }
    ]);
...
const handleSave = async () => {
  ...
  plantRows: plantRows.map(row => ({
    ...
    fromState: row.fromState || "", // NEW: Added fromState
    to: row.to || null,
    toName: row.toName || "",
    taluka: row.taluka || "",
...
//       onChange(rowId, 'fromState', location.state || ''); // Added state from location
//       // Update local status when from state changes
//       updateLocalStatus(rowId);
//     } else if (field === 'to') {
...
// // NEW: Function to update local status
...
//   { key: "localStatus", label: "Local/Not Local", width: "140px" }, // NEW COLUMN
...
//   // ✅ Check if user can see Plant Code / Route section
//   const canSeePlantSection = (email) => {
//     // patilavdhut197@gmail.com - Can see Plant Section
//     if (email === 'patilavdhut197@gmail.com') {
//       return true;
//     }
//     // patilavdhut199@gmail.com - Cannot see Plant Section
//     if (email === 'patilavdhut199@gmail.com') {
//       return false;
//     }
//     // Default: Show for all other users
//     return true;
//   };

//   const showPlantSection = currentUser ? canSeePlantSection(currentUser.email) : true;
...
//     // Only validate plant rows if the section is visible
//     if (showPlantSection) {
//       const hasInvalidPlantRows = plantRows.some(row => !row.plantCode);
//       if (hasInvalidPlantRows) {
//         alert("Please select plant for all plant rows");
...
//         contactPerson: selectedCustomer?.contactPersonName || '',
//         partyName: selectedCustomer?.customerName || top.partyName || '',
//         plantRows: showPlantSection ? plantRows.map(row => ({
//           plantCode: row.plantCode || '',
//           plantName: row.plantName || '',
//           plantCodeValue: row.plantCodeValue || '',
//           orderType: row.orderType || "Sales",
...
//         {/* ✅ PLANT CODE / ROUTE SECTION - Conditionally rendered */}
//         {showPlantSection && (
//           <div className="mt-4">
//             <Card title="Plant Code / Route">
//               <div className="mb-4 flex justify-between items-center">
