...
// // export default function ConsignmentNoteList() {
...
// //       const res = await fetch(`/api/consignment-note?${params.toString()}`, {
...
// //         setNotes(data.data || []);
// //       } else {
// //         setError(data.message || 'Failed to fetch consignment notes');
// //       }
// //     } catch (err) {
// //       console.error('Error fetching consignment notes:', err);
// //       setError('Failed to load consignment notes');
// //     } finally {
// //       setLoading(false);
// //     }
// //   };
...
// //   const handleDelete = async (noteId, lrNo) => {
// //     if (!confirm(`Are you sure you want to delete Consignment Note ${lrNo}?`)) {
// //       return;
// //     }
...
// //       if (data.success) {
// //         setNotes(notes.filter(item => item._id !== noteId));
// //         alert('✅ Consignment Note deleted successfully!');
// //       } else {
// //         alert(data.message || 'Failed to delete consignment note');
// //       }
// //     } catch (err) {
// //       console.error('Error deleting consignment note:', err);
...
// //     router.push(`/admin/Consignment-Note/${noteId}`);
// //   };

// //   const handleCreateNew = () => {
// //     router.push('/admin/Consignment-Note/create');
// //   };
...
// //               Consignment Note (LR) Management
// //             </h1>
// //             <p className="text-sm text-slate-600 mt-1">
// //               Manage all consignment notes in one place
// //             </p>
// //           </div>
...
// //           <h2 className="text-sm font-bold text-slate-700 mb-3">Filter Consignment Notes</h2>
...
// //                   <th className="px-4 py-3 text-left text-xs font-extrabold text-slate-900 uppercase tracking-wider">LR No</th>
...
// //                       <td className="px-4 py-3">
// //                         <div>
// //                           <div className="text-sm text-slate-800">{item.vendorName}</div>
...
// //                         </svg>
// //                         <p className="text-lg font-medium mb-2">No consignment notes found</p>
...
// //               Total {notes.length} consignment note{notes.length !== 1 ? 's' : ''} found
// //             </div>
// //           )}
// //         </div>
// //       </div>
...
//   const MODULE_NAME = 'Consignment Note';

//   // Fetch notes - wrapped in useCallback to prevent infinite re-renders
...
//       alert('You don\'t have permission to delete consignment notes');
//       return;
//     }
...
//       alert('You don\'t have permission to edit consignment notes');
//       return;
//     }
//     router.push(`/admin/Consignment-Note/${noteId}`);
//   };
...
//       alert('You don\'t have permission to create consignment notes');
//       return;
//     }
//     router.push('/admin/Consignment-Note/create');
//   };
...
//   const handleQuickApprove = async (noteId, lrNo) => {
//     if (!canApprove(MODULE_NAME)) {
//       alert('You don\'t have permission to approve consignment notes');
//       return;
//     }

//     if (!confirm(`Are you sure you want to approve Consignment Note ${lrNo}?`)) {
//       return;
//     }

//     setApproveLoading(noteId);
//     try {
//       const token = localStorage.getItem('token');
...
//       if (data.success) {
//         alert('Consignment note approved successfully!');
//         fetchNotes();
//       } else {
...
//             You don't have permission to access Consignment Notes.
...
//                           {canApprove(MODULE_NAME) && item.status !== 'Approved' && item.status !== 'Rejected' && item.status !== 'Completed' && (
//                             <button
//                               onClick={() => handleQuickApprove(item._id, item.lrNo)}
            ...
            <table className="w-full text-sm">
              <thead className="bg-yellow-400 border-b border-yellow-500">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-extrabold text-slate-900 uppercase tracking-wider">LR No</th>
                  <th className="px-4 py-3 text-left text-xs font-extrabold text-slate-900 uppercase tracking-wider">Party Name</th>
                  <th className="px-4 py-3 text-left text-xs font-extrabold text-slate-900 uppercase tracking-wider">LC Status</th>
                    ...
                    <tr key={item._id} className="hover:bg-yellow-50 transition">
                      ...
                      <td className="px-4 py-3">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                          item.lcStatus === 'LC' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'
                        }`}>
                          {item.lcStatus || 'Not LC'}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                          item.lrType === 'Export' ? 'bg-green-100 text-green-800' :
                          item.lrType === 'Import' ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-800'
                        }`}>
                          {item.lrType || 'Normal'}
                        </span>
                      </td>
